...
The key concept is that the vector \emph{z} is passed sequentially from one formula to the next. Each formula's \emph{FormulaCode()} mutates \emph{z} in-place, so the output \emph{z} of formula N becomes the input \emph{z} of formula N+1. This means each formula transforms the result of the previous one, creating complex combined shapes.
...
The sequence of formula execution is determined by the following parameters available on each formula slot:
\begin{itemize}
	\item \emph{Formula} -- which fractal formula to use in this slot
	\item \emph{Iterations} -- how many consecutive iterations this formula runs before advancing to the next slot (default: 1)
	\item \emph{Start at iteration} -- the earliest global iteration number when this formula becomes active (default: 0)
	\item \emph{Stop at iteration} -- the latest global iteration number when this formula is active (default: 250)
	\item \emph{Order} -- which slot the sequence wraps back to when it reaches the end (default: 1)
\end{itemize}

The sequence is generated at scene setup time and maps each global iteration number to a formula slot. The maximum sequence length is 1250 iterations. During rendering, the iteration loop simply looks up which formula to apply for each iteration index.
...
The color coding in the above figure indicates which formula is active:
\begin{itemize}
	\item \textcolor{yellow}{\textcolor{black}{Yellow}} -- slot \#1 (Mandelbulb -- Power~2)
	\item \textcolor{cyan}{\textcolor{black}{Cyan}} -- slot \#2 (Menger Sponge)
	\item \textcolor{green}{\textcolor{black}{Green}} -- slot \#3 (Box Fold Bulb Pow 2)
\end{itemize}
...
{Hybrid sequence result - Simple sequence using three different formulas}
...
This is the number of iterations (repeats) of the formula performed before the loop moves on to the next formula slot in the sequence. If this value is increased to 2 on the first and second formula slots in our example,
...
When computing the iteration loop, the program walks through the enabled formula slots, checking formula iteration range conditions. If the current global iteration number is within the range, then a calculation of that formula is performed and the global iteration count is increased by 1. If the global iteration number is outside the iteration range condition, then the formula is skipped (i.e., no calculation is performed for this slot), but the global iteration count \emph{still increments}. The program then moves to the next enabled slot in the sequence.
...
because \emph{Start at iteration} was set to 4 for this formula, and therefore the slot was skipped. During the second pass of the sequence, the \emph{Menger Sponge} formula was used at iterations 5 and 6,
...
\subsubsection{Formula weight (blending)}

Each formula slot has a \emph{Formula Weight} parameter (range 0.0 to 1.0) that controls how much the formula's output contributes to the result. When the weight is less than 1.0, the output is smoothly blended between the formula's result and the previous state of \emph{z}. This allows for gradual transitions between formulas and can produce unique visual effects.

\subsubsection{Order (repeat from)}

The \emph{Order} parameter (labeled \emph{repeat\_from} internally) determines where the sequence wraps back to when it reaches the last formula slot. The value ranges from 1 to 9, where 1 means the sequence restarts from the first formula (default behavior). Setting it to 2, for example, makes the sequence wrap back to the second formula instead, creating a pattern like: A, B, C, B, C, B, C, ...

This parameter affects the long-term structure of the hybrid fractal and can be used to create asymmetric or repeating patterns.
