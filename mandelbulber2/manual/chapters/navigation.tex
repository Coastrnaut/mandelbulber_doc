...
The \emph{camera\_top} vector defines the upward direction of the camera. This vector is automatically updated when the camera is rotated. It is exposed in the keyframe animation editor but not in the main navigation controls.
...
\begin{description}
	\item[Left mouse button] rotates camera by moving target
	\item[Right mouse button] rotates camera around indicated point by moving camera and target
	\item[Middle button] rotates camera by changing only the roll angle
	\item[Left + right buttons] moves camera and target relative to the indicated point
	\item[Control key + mouse wheel] moves camera forward/backward towards indicated point
\end{description}

\subsection{Camera and Target movement step}\label{camera-and-target-movement-step}
...
\subsection{Linear camera and target movement modes using the arrow buttons}\label{linear-camera-and-target-movement-modes-using-the-arrow-buttons}

A user can navigate by operating the arrow buttons on the Navigation dock, with the
user defined steps.

The \emph{Camera movement mode} parameter controls the relationship between camera and target during arrow key navigation. It has three modes:

\begin{description}
	\item[Fixed distance] move camera and target together (default)
	\item[Move camera] move only camera
	\item[Move target] move only target
\end{description}

These modes are available in the Navigation dock.
...
This feature is controlled by the click mode setting. When click modes are enabled, clicking on the rendered image initiates camera movement toward the clicked point.
...
\subsection{Camera rotation modes using the arrow buttons}\label{camera-rotation-modes-using-the-arrow-buttons}
...
moved accordingly (figure \ref{rotate_camera}). This is the standard mode for rotating the camera.
...
Reset View zooms the camera out from the fractal while maintaining the camera's orientation (angles). The target is reset to $(0, 0, 0)$ and the camera is positioned along the current forward vector at a distance calculated by scanning the fractal size in multiple random directions.
...
\subsection{Calculation of rotation angle modes}\label{calculation-of-rotation-angle-modes}

The \emph{Rotation mode} parameter (labeled in the Navigation dock) controls how rotation angles are calculated. It has two options:
...
This mode can be likened to an aircraft's controls, where all turns are relative
...
left always gives the visual effect of the camera rotating in that direction. The
...
The following keyboard shortcuts are available for camera navigation (default bindings, configurable in settings):

\begin{description}
	\item[Q] move forward
	\item[Z] move backward
	\item[A] move left
	\item[D] move right
	\item[W] move up
	\item[S] move down
	\item[Left arrow] rotate left
	\item[Right arrow] rotate right
	\item[Up arrow] rotate up
	\item[Down arrow] rotate down
\end{description}
...
To compensate for this, on the Keyframe navigation tab use the button \emph{Set constant target distance}. This adjusts all keyframes by
